...
public class NodeManager {
	...
	//Master node is hardcoded for all nodes, after first run of the instance set it on empty string
	...
	private void init() {
		try {
			...
			if (master != null && !master.equals("") && !master.equals(this.node.getAddress())) {
				...
				ResteasyWebTarget rtarget = client.target("http://" + master + "/project-war/rest/connection");
			...
			heartBeat.scheduleAtFixedRate(new TimerTask() {
			    ...
			    public void run() {
			        ...
			        for (Node n: nodes) {
			        	...
			        	try {
			        		ResteasyWebTarget rtarget = client.target("http://" + n.getAddress() + "/project-war/rest/connection");
							ServersRestRemote rest = rtarget.proxy(ServersRestRemote.class);
			        		...
			        		try {
				        		...
				        		for (Node n1: nodes) {
				        			...
				        			ResteasyWebTarget rtarget = client.target("http://" + n1.getAddress() + "/project-war/rest/connection");
				    				ServersRestRemote rest = rtarget.proxy(ServersRestRemote.class);
				    				rest.deleteNode(n.getAlias());
				        		}
				        		if (!getNode().getAddress().equals(getMaster())) {
				        			ResteasyWebTarget rtarget = client.target("http://" + getMaster() + "/project-war/rest/connection");
				        			ServersRestRemote rest = rtarget.proxy(ServersRestRemote.class);
				        			rest.deleteNode(n.getAlias());
				        		}
