...
@Path("/connection")
@LocalBean
public class ServersRest implements ServersRestLocal, ServersRestRemote{
	...
	public void informNodesAboutLoggedInUsers() {
		...
		for (Node node: nodeManager.getNodes()) {
			...
			ResteasyWebTarget rtarget = client.target("http://" + node.getAddress() + "/project-war/rest/connection");
			ServersRestRemote rest = rtarget.proxy(ServersRestRemote.class);
			rest.setLoggedIn(data.getLoggedInUsers());
		}
		if (!nodeManager.getNode().getAddress().equals(nodeManager.getMaster())) {
			ResteasyWebTarget rtarget = client.target("http://" + nodeManager.getMaster() + "/project-war/rest/connection");
			ServersRestRemote rest = rtarget.proxy(ServersRestRemote.class);
			rest.setLoggedIn(data.getLoggedInUsers());
		}
	}
